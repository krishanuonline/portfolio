...
const Header = () => {
  ...
  return (
    <header 
      ...
      ${isScrolled 
        ? 'bg-dark-base/90 backdrop-blur-md border-surface shadow-lg' 
    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <nav className="hidden lg:flex items-center gap-1 xl:gap-2">
            {navLinks.map((link) => (
              <a 
                key={link.name}
                href={link.href} 
                className="px-3 py-2 text-sm font-medium text-text-secondary transition-all hover:text-primary rounded-lg hover:bg-surface"
              >
                {link.name}
              </a>
            ))}
          ...
          <div className="hidden lg:block">
            <button className="bg-primary hover:bg-primary-hover text-white px-5 py-2 rounded-xl text-sm font-bold transition-all active:scale-95 shadow-[0_0_20px_rgba(97,86,226,0.3)]">
              Contact
            </button>
          </div>
      ...
      {/* Mobile Menu Overlay */}
      <div className={`lg:hidden overflow-hidden transition-all duration-300 ease-in-out bg-surface border-b border-white/5 
        ...
        <nav className="flex flex-col p-4 space-y-2">
          {navLinks.map((link) => (
            <a 
              ...
              className="px-4 py-3 text-base font-medium text-text-secondary hover:text-text-primary hover:bg-primary/10 rounded-xl transition-colors"
            >
              {link.name}
            </a>
          ))}
